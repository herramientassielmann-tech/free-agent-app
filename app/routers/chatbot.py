...
async def chatbot_new(
    ...
    db.commit()
    db.refresh(conv)
    return RedirectResponse(url=f"/admin/chatbot/{conv.id}", status_code=303)
...
async def chatbot_update_context(
    ...
    db.commit()
    return RedirectResponse(url=f"/admin/chatbot/{cid}", status_code=303)
...
async def chatbot_delete(
    ...
    db.delete(conv)
    db.commit()
    return RedirectResponse(url="/admin/chatbot", status_code=303)
